...
namespace AuthTokenServiceCore.Helpers
{
    ...
    public static class Saml2TokenExtensions
    {
        ...
        {
            Saml2SubjectConfirmationData confirmationData = new Saml2SubjectConfirmationData() { Address = ApplicationSettings.ApplicationOptions.ConfirmationMethod };
            Saml2SubjectConfirmation subjectConfirmations = new Saml2SubjectConfirmation(new Uri(ApplicationSettings.ApplicationOptions.ConfirmationMethod), confirmationData);
            Saml2AudienceRestriction[] audienceRestriction = new Saml2AudienceRestriction[1] { new Saml2AudienceRestriction(ApplicationSettings.ApplicationOptions.AudienceUri.ToString()) };
            Saml2Assertion assertion = new Saml2Assertion(new Saml2NameIdentifier(ApplicationSettings.ApplicationOptions.Issuer))
            {
                ...
                InclusiveNamespacesPrefixList = ApplicationSettings.ApplicationOptions.Namespace,
                Subject = new Saml2Subject(subjectConfirmations)
                {
                    NameId = new Saml2NameIdentifier(ApplicationSettings.ApplicationOptions.ConfirmationMethod)
                }
            };
        ...
        private static bool IsValidToken(Saml2SecurityToken saml2Token)
        {
            ...
            return isValidIssueDate && isValidExpirationDate && ApplicationSettings.ApplicationOptions.Issuer == saml2Token.Assertion.Issuer.Value;
        }
        ...
        {
            int validForFinal = ApplicationSettings.ApplicationOptions.ValidFor;
            DateTime utcNow = DateTime.UtcNow;
